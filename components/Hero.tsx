...
const Hero: React.FC = () => {
  ...
  const scrollToContent = () => {
    // Scroll past the hero section, accounting for the sticky header
    window.scrollTo({
  ...
  return (
    <section className="relative w-full h-[calc(100vh-80px)] bg-white flex flex-col justify-center overflow-hidden">
      <div className="w-full max-w-[1600px] mx-auto px-6 md:px-12 lg:px-16 flex flex-col items-start leading-[0.85] select-none py-10">
      ...
      <button 
        ...
        className={`absolute bottom-10 left-6 md:left-12 lg:left-16 flex items-center gap-3 group cursor-pointer transition-all duration-1000 delay-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
