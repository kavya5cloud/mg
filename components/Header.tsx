...
import { Menu, X, Search, Ticket, ArrowRight, ChevronRight, Heart } from 'lucide-react';
import { Link, useLocation, useNavigate } from 'react-router-dom';
import LogoGeometric from './Logo';
import { EXHIBITIONS, ARTWORKS } from '../constants';
...
const Header: React.FC = () => {
  ...
  const searchResults = useMemo(() => {
    ...
    const pages = [
      ...
      { title: 'Calendar & Events', path: '/calendar', type: 'Page' },
    ...
    EXHIBITIONS.forEach(ex => {
      if (ex.title.toLowerCase().includes(query) || ex.description.toLowerCase().includes(query)) {
        results.push({
          title: ex.title,
          path: '/exhibitions', // In a real app, this might be /exhibitions/:id
          type: 'Exhibition',
          meta: ex.dateRange
        });
      }
           ...
           <div className="max-w-[1000px] mx-auto py-8 px-6">
              {searchResults.length > 0 ? (
                ...
                <div className="text-center py-12 text-gray-400">
                   ...
                   <p className="text-sm">Try searching for "Video", "Van Gogh", or "Tickets"</p>
                </div>
              )}
           </div>
        </div>
