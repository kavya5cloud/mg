...
import { Lock, Search, RefreshCw, LogOut, TrendingUp, Users, Calendar, Pen, Trash, Plus, ShoppingBag, Image as ImageIcon, Check, X, Package, Filter, Upload, Mail, BarChart3, PieChart } from 'lucide-react';
import { Link } from 'react-router-dom';
import { getExhibitions, saveExhibitions, getArtworks, saveArtworks, getCollectables, saveCollectables, getShopOrders, updateShopOrders, getNewsletterEmails, getBookings } from '../services/data';
import { Exhibition, Artwork, Collectable, ShopOrder, Booking } from '../types';

type Tab = 'bookings' | 'orders' | 'exhibitions' | 'collection' | 'shop' | 'newsletter';
      ...
      <div className="space-y-3">
          ...
          >
              ...
              {editItem.imageUrl ? (
                  <div className="relative w-full aspect-video bg-gray-200 rounded-lg overflow-hidden shadow-sm">
                      <img src={editItem.imageUrl} alt="Preview" className="w-full h-full object-contain bg-[url('https://transparent-textures.patterns.velomo.de/subtle_white_feathers.png')]" />
                      ...
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center text-white transition-opacity duration-200">
                          ...
                          <span className="font-bold text-sm">Click or Drop to Replace</span>
                      </div>
                  </div>
              ) : (
                  <div className="py-4">
                      ...
                      <p className="text-[10px] text-gray-400 mt-1 font-medium">SVG, PNG, JPG, GIF (Max 5MB)</p>
                  </div>
              )}
          </div>
          ...
          <div className="relative">
               ...
               <input 
                  ...
                  className="w-full border border-gray-200 pl-10 pr-10 py-2.5 rounded-lg bg-white text-sm focus:border-black focus:ring-1 focus:ring-black outline-none transition-all placeholder:text-gray-400"
          ...
          {isDataUrl && <p className="text-[10px] text-green-600 font-bold flex items-center gap-1"><Check className="w-3 h-3" /> Image uploaded successfully</p>}
      </div>
    );
  };
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h3 className="text-lg font-bold flex items-center gap-2"><BarChart3 className="w-4 h-4" /> Weekly Visitor Forecast</h3>
                                <p className="text-xs text-gray-400">Projected guests for next 7 days</p>
                            </div>
                        </div>
                        <div className="flex items-end justify-between h-48 gap-3 pt-4 border-b border-gray-100">
                             {analyticsData.map((d, i) => (
                                <div key={i} className="flex flex-col items-center gap-2 flex-1 group cursor-pointer">
                                    <div className="w-full relative flex items-end justify-center h-full">
                                        <div 
                                            style={{ height: `${Math.max((d.value / maxChartValue) * 100, 4)}%` }} 
                                            className={`w-full max-w-[40px] rounded-t-sm transition-all duration-500 relative ${i === 0 ? 'bg-black' : 'bg-gray-200 group-hover:bg-gray-400'}`}
                                        >
                                    ...
                                    <span className={`text-[10px] font-bold uppercase ${i === 0 ? 'text-black' : 'text-gray-400'}`}>{d.label}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col">
                        <div className="mb-6">
                            <h3 className="text-lg font-bold flex items-center gap-2"><PieChart className="w-4 h-4" /> Demographics</h3>
                            <p className="text-xs text-gray-400">Visitor type breakdown</p>
                        </div>
                        <div className="flex-grow flex flex-col justify-center space-y-6">
                            <div>
                                <div className="flex justify-between text-sm font-bold mb-1">
                                    <span>Adults</span>
                                    <span>{Math.round((visitorTypes.adult / (stats.visitors || 1)) * 100)}%</span>
                                </div>
                                <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
                                    <div className="bg-black h-full rounded-full" style={{ width: `${(visitorTypes.adult / (stats.visitors || 1)) * 100}%` }}></div>
                                </div>
                                <p className="text-xs text-gray-400 mt-1">{visitorTypes.adult} Total</p>
                            </div>
                            <div>
                                <div className="flex justify-between text-sm font-bold mb-1">
                                    <span>Students</span>
                                    <span>{Math.round((visitorTypes.student / (stats.visitors || 1)) * 100)}%</span>
                                </div>
                                <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
                                    <div className="bg-gray-500 h-full rounded-full" style={{ width: `${(visitorTypes.student / (stats.visitors || 1)) * 100}%` }}></div>
                                </div>
                                <p className="text-xs text-gray-400 mt-1">{visitorTypes.student} Total</p>
                            </div>
                            <div>
                                <div className="flex justify-between text-sm font-bold mb-1">
                                    <span>Children</span>
                                    <span>{Math.round((visitorTypes.child / (stats.visitors || 1)) * 100)}%</span>
                                </div>
                                <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
                                    <div className="bg-gray-300 h-full rounded-full" style={{ width: `${(visitorTypes.child / (stats.visitors || 1)) * 100}%` }}></div>
                                </div>
                                <p className="text-xs text-gray-400 mt-1">{visitorTypes.child} Total</p>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {bookings.map((booking) => (
                                    <tr key={booking.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 font-bold">{booking.totalAmount === 0 ? 'Free' : `₹${booking.totalAmount}`}</td>
